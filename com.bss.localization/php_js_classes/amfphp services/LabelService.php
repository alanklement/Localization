...
class LabelService
{
	...
	{		
		$utils = new ResultUtils() ;
			...
			if ( $projectID == -2 )
			{
					$query = mysql_query( "SELECT t1.id as label_id, t1.label, t1.edited, t1.char_limit, t2.phrase as original
					FROM
					( select * from label) as t1
					left join 
					( select l.id, ph.phrase
					from label l, label_phrase l_ph, phrase ph
					where l.id = l_ph.label_id
					AND l_ph.phrase_id = ph.id
					and l_ph.orig = 1) as t2 on t1.id=t2.id") ;
			}
			else if( $projectID == -1 )
			{
	...
	{		
		...
		$query = mysql_query( "SELECT phrase.id as translation_id, phrase.edited as phrase_edited FROM phrase where phrase.id = '$createdPhraseID'" )	;
		if ( !$query ) {  $result = $utils->fail('Server failed to retreive your created phrase') ; }
		else if ( $query ) { $result = $utils->createPhraseResultVOs($query) ;}
